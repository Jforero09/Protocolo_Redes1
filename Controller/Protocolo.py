class Protocolo:
    ...
    def crear_trama(self, delimitador_inicial, cof, fin, sol, ctr, per, num, data, delimitador_final):
        if delimitador_inicial != self.delimitador or delimitador_final != self.delimitador:
            print("Los delimitadores son incorrectos, deben ser 10101010")
        else:
            return {
    ...
    def enviar_trama(self, trama):
        ...
        if (len(self.tramas_recibidas) > 0 and
            ...
            (trama['cof'], trama['sol'], trama['ctr'], trama['per'], trama['fin']) == (0, 0, 0, 0, 0) and
            trama['data'] == self.mensaje_transmitir[trama['num']-1] and
            trama['num'] < len(self.mensaje_transmitir)-1 and
            trama['num'] == 1):
            self.tramas_enviadas.append(trama)
            return True
        if (self.tramas_enviadas[0] == self.solicitud_permiso and
            (trama['cof'], trama['sol'], trama['ctr'], trama['per'], trama['fin']) == (0, 0, 0, 0, 0) and
            trama['data'] == self.mensaje_transmitir[trama['num'] - 1] and
            trama['num'] < len(self.mensaje_transmitir) and
            trama['num'] > 1 and
            self.tramas_recibidas[-1]['cof'] == 1 and self.tramas_recibidas[-1]['ctr'] == 1):
            self.tramas_enviadas.append(trama)
            return True
        if ((trama['cof'], trama['sol'], trama['ctr'], trama['per']) == (0, 0, 0, 0) and
            trama['fin'] == 1 and
            trama['data'] == self.mensaje_transmitir[trama['num'] - 1] and
            trama['num'] == len(self.mensaje_transmitir) and
            ...
            return True
        if (
            trama['fin'], trama['ctr'] == (1,1) and
            trama['cof'], trama['sol'], trama['per'], trama['num'] == (0, 0, 0, 0) and
            trama['data'] == '' and
    ...
    def responder_trama(self, trama):
        if trama == self.permiso_concedido and self.tramas_enviadas[0] == self.solicitud_permiso:
            self.tramas_recibidas.append(trama)
            return True
        if (trama['cof'] == 1 and trama['ctr'] == 1 and
